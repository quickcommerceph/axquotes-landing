...
import { useRef, useEffect, type CSSProperties, type MouseEventHandler, type ReactNode, type Ref } from 'react';
...
} as const satisfies Record<string, SpecularButtonVariant>;

interface SpecularButtonProps extends SpecularButtonVariant {
  children?: ReactNode;
  href?: string;
  size?: 'sm' | 'md' | 'lg';
  radius?: number;
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // Pauses the render loop and the page-wide pointermove listener while the
    // button is off-screen, since each instance owns its own WebGL context.
    const start = () => {
      if (running) return;
      running = true;
  ...
  const label = (
    <span className="relative z-[2] inline-flex items-center gap-[0.7rem] [&_svg]:h-[1.05rem] [&_svg]:w-[1.05rem]">
      {children}
    </span>
  );
